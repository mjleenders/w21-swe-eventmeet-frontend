    ...
    <div class="tab-content">
        <div class="text row tab-pane fade show active" id="home">
            <div class="row justify-content-center py-3">
                <h1 class="text-center">{{movie.event.title}}</h1>
                <div class="list-inline text-center">
                    <h5 class="list-inline-item" v-for="genre in movie.event.genres" :key="genre">{{genre}}</h5>
                </div>
                <div class="col-md-auto">
                    <ul class="list-group list-group-flush list-group-horizontal">
                        <li class="list-group-item ">{{movie.event.regisseur}}</li>
                        <li class="list-group-item">{{movie.event.released}}</li>
                        <li class="list-group-item">+{{movie.event.fsk}}</li>
                        <li class="list-group-item">{{movie.event.length}}min</li>
                    </ul>
                </div>
            </div>
            <div class="media d-flex">
                <img class="media-object border border-light" :src="movie.event.logos[0].ref" alt="">
                <div class="media-body py-3 px-3 shadow-lg rounded bg-dark">
                    <h5 class="media-heading">Beschreibung:</h5>
                    <p>{{movie.event.description}}</p>
                </div>
            </div>

            <div class="row my-4">
                <div class="ratio ratio-16x9">
                    <iframe :src="movie.event.trailer" title="trailer" allowfullscreen></iframe>
                </div>
            </div>

            <div class="row my-4">
                <Kommentare :allratings="ratings" :EventID="$route.params.MovieID"/>
            </div>
        </div>
...
export default {
    ...
    async created(){
        const response = await axios.get('/events/movies/'+this.$route.params.MovieID);
